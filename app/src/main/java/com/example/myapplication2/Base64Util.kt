package zebpay.application.utils
...
object Base64Util {
     ...
     * @since 1.4
     */
    fun enCodeByArrayToBase64(source: ByteArray): String {
        return enCodeByArrayToBase64(source, 0, source.size, ALPHABET, true)
    }
    ...
    fun encodeWebSafe(source: ByteArray, doPadding: Boolean): String {
        return enCodeByArrayToBase64(source, 0, source.size, WEBSAFE_ALPHABET, doPadding)
    }
     ...
     * @since 1.4
     */
    fun enCodeByArrayToBase64(
        source: ByteArray, off: Int, len: Int, alphabet: ByteArray,
        doPadding: Boolean
    ): String {
        val outBuff = enCodeByArrayToBase64(source, off, len, alphabet, Int.MAX_VALUE)
    ...
    fun enCodeByArrayToBase64(
        ...
        while (d < len2) {
            ...
            val inBuff: Int = (source[d + off].toInt() shl 24 ushr 8
    ...
    private fun decode4to3(
        ...
        return if (source[srcOffset + 2] == EQUALS_SIGN) {
            val outBuff: Int = (decodabet[source[srcOffset].toInt()].toInt() shl 24 ushr 6
    ...
    fun decpdeBase64ToByteArray(s: String): ByteArray {
        val bytes = s.toByteArray()
        return decpdeBase64ToByteArray(bytes, 0, bytes.size)
    }
    ...
    @JvmOverloads
    @Throws(Base64DecoderException::class)
    fun decpdeBase64ToByteArray(
        source: ByteArray,
        off: Int = 0,
        len: Int = source.size,
        decodabet: ByteArray = DECODABET
    ): ByteArray {
        val len34 = len * 3 / 4
            ...
            if (sbiDecode >= WHITE_SPACE_ENC) { // White space Equals sign or better
                if (sbiDecode >= EQUALS_SIGN_ENC) {
                    ...
                    if (sbiCrop == EQUALS_SIGN) {
                        val bytesLeft = len - i
                        val lastByte: Byte = (source[len - 1 + off] and 0x7f).toByte()
                ...
                throw Base64DecoderException(
                    "Bad Base64Util input character at " + i
